...
const sumaNotas=exams.reduce((acc,notas)=>acc + notas.score,0);
...
const sumaNotas2=exams.reduce((acc,notas)=>{
if (acc + notas.score >5){
  return acc+notas.score;
}else{
  return acc
}
} ,0);
...
const media=sumaNotas.length
const mediaExams=exams.reduce((acc,notas)=> acc + notas.score,0)/ exams.length
